...
def _load_deps():
    try:
        ...
        raise SystemExit(
            "Missing inference dependencies. Install with: python3 -m pip install -e '.[train]'"
        ) from exc
    return torch, AutoModelForImageTextToText, AutoTokenizer, PeftModel
...
def _make_db(prompt: str) -> sqlite3.Connection:
    ...
    for table, body in CREATE_TABLE_RE.findall(prompt):
        create_sql = f"CREATE TABLE {table} ({body});"
...
def _load_model(model_name: str, adapter: str | None, dtype_arg: str):
    torch, AutoModelForImageTextToText, AutoTokenizer, PeftModel = _load_deps()
...
def _predict(torch, tokenizer, model, device: str, record: dict[str, Any], max_new_tokens: int) -> tuple[str, float]:
    try:
        ...
        prompt = tokenizer.apply_chat_template(_prompt_messages(record), tokenize=False, add_generation_prompt=True)
...
def _run_label(label: str, records: list[dict[str, Any]], args, adapter: str | None) -> dict[str, Any]:
    torch, tokenizer, model, device = _load_model(args.model, adapter, args.dtype)
    predictions = []
    start = time.time()
    for index, record in enumerate(records, start=1):
        prediction, elapsed = _predict(torch, tokenizer, model, device, record, args.max_new_tokens)
    ...
    scored["summary"]["mean_latency_seconds"] = round(
        sum(item["latency_seconds"] for item in predictions) / len(predictions), 4
...
def main() -> int:
    ...
    parser.add_argument("--dtype", choices=["auto", "float16", "float32"], default="float32")
